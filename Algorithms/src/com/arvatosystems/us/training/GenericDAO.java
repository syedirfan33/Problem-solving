...
// This DAO class id an Hibernate Entity, which is persisted into Database containing methods to return list of entites based on search criteria.
public abstract class GenericDAO<E extends Entity>
{
	...
	// This method returns List of all the entities starting from an index/row "start" and result set count is  "count"
	public List<E> findAll(final int count, final int start)
	{
		return findAllByAttributes(null, null, count, start, false);
	}

	// This method returns List of all the entities matching attributes "attribs", order the list by a "String", Starts from a row number "start" and returns results of count "count", provided that if exclusion is required or not.
